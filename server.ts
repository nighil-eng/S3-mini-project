...
import {
  classifyTransportModeFromGps,
} from './src/lib/haversine';
...
async function startServer() {
  ...
  /*
   * Hosting services such as Render provide PORT
   * automatically through process.env.PORT.
   *
   * Local development will use port 3000.
   */
   ...
   *
   * You can override this later using:
   *
   * FRONTEND_URL=https://nighil-eng.github.io
   */
  const FRONTEND_URL =
    process.env.FRONTEND_URL ||
    'https://nighil-eng.github.io';
  ...
  try {
    ...
    console.log(
      'Server will continue using the local/default data configuration.'
    );
  }
   ...
   * JSON body parser.
   *
   * Image data can be large, so the limit is 15 MB.
   */
  app.use(
    express.json({
      limit: '15mb',
  ...
  /*
   * GitHub Pages and your backend are different origins.
   * Therefore the browser requires CORS permission.
   */
  ...
  app.use((req, res, next) => {
    ...
    if (
      ...
      /*
       * For the current project we also allow the GitHub Pages
       * subdomain.
       */
      if (
        requestOrigin &&
        requestOrigin.includes('nighil-eng.github.io')
      ) {
        res.header(
          'Access-Control-Allow-Origin',
          requestOrigin
  ...
  /*
   * Open this URL after deploying the backend:
   *
  ...
  app.get('/api', (req, res) => {
    res.json({
      success: true,
      message: 'EcoTrack AI API is running',
      version: '1.0.0',
    });
  });
  ...
  app.put('/api/profile', (req, res) => {
    try {
      const {
        targetMonthlyFootprint,
        ecoGoal,
        country,
      } = req.body;

      if (
        targetMonthlyFootprint !== undefined &&
        targetMonthlyFootprint !== null
      ) {
        DEFAULT_PROFILE.targetMonthlyFootprint =
          Number(targetMonthlyFootprint);
      }

      if (ecoGoal) {
        DEFAULT_PROFILE.ecoGoal = ecoGoal;
      }

      if (country) {
        DEFAULT_PROFILE.country = country;
      }
  ...
  app.post('/api/electricity/ocr', async (req, res) => {
    try {
      ...
      const result =
        await ocrElectricityBill(
          imageBase64,
          mimeType || 'image/png'
        );

      /*
       * CO2 = electricity units × grid emission factor
       */
      const unitsKwh =
        Number(result.unitsKwh) || 150;

      const co2Kg = Number(
        (
          unitsKwh *
          DEFAULT_PROFILE.gridFactorKgPerKwh
        ).toFixed(2)
      );
      ...
      const newBill = {
        ...
        co2Kg,
        billAmount:
          Number(result.billAmount) || 45,
        utilityProvider:
          result.utilityProvider ||
          'Clean Grid Electric',
        rawOcrText:
          result.rawOcrText,
        createdAt:
  ...
  app.delete(
    ...
    (req, res) => {
      const index =
        ELECTRICITY_BILLS.findIndex(
          (bill) =>
            bill.id === req.params.id
        );

      if (index !== -1) {
        ELECTRICITY_BILLS.splice(
          index,
          1
        );
      }
      ...
      try {
        ...
        if (
          ...
          return res.status(400).json({
            success: false,
            error:
              'At least 2 GPS coordinates are required to classify trip.',
          });
        }

        /*
         * Transport mode is determined from
         * GPS trajectory.
         */
        const summary =
          classifyTransportModeFromGps(
  ...
  app.get(
    ...
    (req, res) => {
      try {
        ...
        res.json({
          title:
            'EcoTrack AI Sustainability Audit & Carbon Report',
  ...
  /*
   * When running locally, Vite serves the React application.
   *
   * When running production, Express can serve the built
   * frontend from dist.
   *
   * GitHub Pages will normally serve the frontend separately.
   */
  ...
  app.listen(
    ...
    () => {
      console.log(
        `EcoTrack AI Express Server running on port ${PORT}`
      );
...
startServer().catch((error) => {
  console.error(
    'Failed to start EcoTrack AI server:',
    error
  );
